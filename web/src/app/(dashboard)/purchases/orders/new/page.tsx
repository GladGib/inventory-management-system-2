...
export default function NewPurchaseOrderPage() {
  ...
  const subtotal = items.reduce((sum, item) => sum + (item.quantity * (item.unitCost || 0)), 0);
  const totalDiscount = items.reduce(
    (sum, item) => sum + (item.quantity * (item.unitCost || 0) * (item.discountPercent || 0) / 100),
    0
  );
  const totalTax = items.reduce((sum, item) => {
    const afterDiscount = item.quantity * (item.unitCost || 0) * (1 - (item.discountPercent || 0) / 100);
